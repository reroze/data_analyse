...
def eval_data_mini(test_data, matrix, alias_movie, mini_matrix, alias_movie_reverse, k_times):
    test_users = []
    test_movies = []
    real_result = []
    ...
    pre_result = []
    for i in range(len(test_data)):
        pre_result.append(eval_single_mini(test_users[i], test_movies[i], matrix, mini_matrix, k_times))
...
for k_times in range(3, 50):
    loss = eval_data_mini(test_data, matrix, alias_movie, mini_matrix, alias_movie_reverse, k_times)
...
plt.show()
print(min(y_label))
print(y_label.index(min(y_label)))
print('times {}ms'.format(end-begin))
'''



item_list_pre = predict_mini(452, matrix, mini_matrix, 7, 3)
#print(item_list_pre)
true_item_list_pre = []
